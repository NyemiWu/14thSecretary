...
| 想干什么 | 怎么进 | 会发生什么 |
|---|---|---|
| **改已有条目** | 在站上翻到那一页，点标题下面的 **编辑此条目** | 编辑器自动把原文拉进来给你改。提交后进缓冲区，**原文一个字都不动** |
| **新建条目** | 直接开 `/editor/`，或在板块落地页点 **在此分区新建条目** | 空表单，选好分区就行 |

两个入口走的是**同一套流程、同一份表单**，最后都落到 `buffer/pending/`。

!!! tip "为什么「改条目」不直接改原文"
    缓冲区的意义就在这儿 —— 谁都能提改动，但改动要经管理员确认才生效。
    所以「修改稿」进缓冲区时带一个 `revises` 字段记着改的是哪一篇，
    正文是**完整的新版本**；通过并公示完，管理员拿它替换掉原文件。

!!! warning "板块落地页上没有「编辑此条目」"
    `/text/worldview/` 这类聚合页是 `scripts/gen_section_index.py` 生成的，
    手改会在下次构建时被覆盖。所以那些页面上的按钮是**在此分区新建条目**。
    要改介绍文字，改脚本里的 `SECTIONS`；要改条目，点进具体那一篇。
...
| 字段 | 说明 |
|---|---|
| 所属分区 | 下拉选。选完「拟并入位置」自动算出来 |
| 稿件编号 | 三位数，**已自动取下一个可用号**。例 `012` |
| 英文短名 | 小写英文加短横线。例 `ember-pact`。**只用于文件名和网址**，页面不显示 |
| 标题 | 中文标题 |
| 提交人 | 你的代号，会显示在公示里。填过一次浏览器会记住 |
| 提交日期 | 默认今天 |
| 拟并入的位置 | 例 `text/canon/xxx.md`。改条目时这里是原文路径，一般不用动 |
